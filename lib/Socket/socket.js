...
const makeSocket = (config) => {
    ...
    authState.creds.me = {
        id: phoneNumber.replace(/\D/g, '') + '@s.whatsapp.net',
        name: '~'
    };
    ...
    // logic so as not to log out
ws.on('CB:failure', (node) => {
    const reason = +(node.attrs.reason || 500);

    if (reason === 401) {
        logger.warn("401 (payload ditolak Whatsapp), abaikan logout & reconnect otomatis");

        ev.emit('connection.update', {
            connection: 'close',
            lastDisconnect: {
                error: new Error('Ignored 401 Failure'),
                date: new Date()
            }
        });
        ...
        setTimeout(() => {
            try {
                exports.makeSocket(config); // redial connection
            } catch (err) {
                logger.error("Gagal reconnect 401:", err);
            }
        }, 3000);
